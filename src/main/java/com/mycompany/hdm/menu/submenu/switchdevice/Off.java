 ...
 */
public class Off implements MenuItems {
    ...
    @Override
    public void execute() {
        logger.info("Was selected menu item : OFF");
        devices.clear();
        try {
        ...
        do {
            ...
            if ((!input.equalsIgnoreCase(SUBITEMS.EXIT.toString()) && !input.equalsIgnoreCase(SUBITEMS.BACK.toString()))) {
                ...
                try {
                    (uid.meetCriteria(devices, "homeDev" + input).get(0)).switchOFF();
    ...
    private void showOptions() {
        ...
        for (HomeDevices dev : devices) {
            if (dev.getStatus().equals(HomeDevices.Status.ON)) {
